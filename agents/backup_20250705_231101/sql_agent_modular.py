    ...
    def _extract_query_params(self, query: str, template: Any, params: Dict,
        ...
        try:
            ...
            if extracted_params.error:
                self.logger.error(f"参数提取错误: {extracted_params.error}")
                return {}
            ...
            if template.requires_stock and extracted_params.stocks:
                ...
                if entities:
                    ...
                    if success:
                        ...
                        self.logger.error(f"股票验证失败: {error_msg}")
                        return {}
                else:
                    self.logger.error("未识别到股票信息")
                    return {}
            
            # 处理日期参数
            if template.requires_date if hasattr(template, 'requires_date') else True:
            ...
            if '排名' in template_name or '排行' in template_name:
                if 'trade_date' not in query_params and 'start_date' not in query_params:
                    # 获取最新交易日
                    last_trading_date = self._get_last_trading_date()
                    query_params['trade_date'] = last_trading_date
                    self.logger.debug(f"为排名查询添加默认交易日: {last_trading_date}")
