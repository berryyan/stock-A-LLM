"""
智能处理器 V5.3 - Milvus查询优化版
主要更新：
1. 修复Milvus查询10000条限制问题，支持分批查询
2. 添加日期范围过滤优化查询性能
3. 改进已处理记录的验证逻辑
4. 保持V5.2所有功能（季度报告日期范围处理、控制台优化等）
"""
import sys
import os
...
class ProcessedRecordManager:
    """已处理记录管理器 - V5.3改进版，支持与Milvus同步"""
    
    def __init__(self, milvus_connector=None):
        self.record_file = Path("data/processed_announcements.json")
        self.milvus = milvus_connector
        self.processed_ids = self._load_records()
        self._changes_count = 0  # 记录变更次数
        self._last_sync_time = 0  # 上次与Milvus同步时间
    ...
    def sync_with_milvus(self, start_date: str = None, end_date: str = None, force: bool = False):
        """与Milvus同步已处理记录
        ...
        """
        if not self.milvus:
            logger.warning("未提供Milvus连接器，无法同步")
            return
            
        # 如果不是强制同步，且距离上次同步不到1小时，则跳过
        if not force and time.time() - self._last_sync_time < 3600:
            logger.info("距离上次同步不到1小时，跳过")
            return
            
        print("\n与Milvus同步已处理记录...", end='', flush=True)
        
        # 从Milvus获取已处理ID
        milvus_ids = self._get_processed_from_milvus(start_date, end_date)
        
        # 合并ID
        before_count = len(self.processed_ids)
        self.processed_ids.update(milvus_ids)
        ...
        print(f" 完成，新增 {added_count} 条记录")
        
    def _get_processed_from_milvus(self, start_date: str = None, end_date: str = None) -> Set[str]:
        """从Milvus获取已处理的公告ID（分批查询）"""
        processed_ids = set()
        batch_size = 5000
        offset = 0
        max_offset = 16384 - batch_size  # Milvus限制
        ...
        try:
            while offset <= max_offset:
                # 调整最后一批的大小
                current_limit = min(batch_size, 16384 - offset)
                ...
                # 显示进度
                print(f"\r与Milvus同步已处理记录... {len(processed_ids)} 条", end='', flush=True)
                
                # 如果返回数量少于limit，说明已经是最后一批
                ...
                # 达到限制时警告
                if offset >= max_offset and len(results) == current_limit:
                    logger.warning("达到Milvus查询限制(16384条)")
                    break
                
        except Exception as e:
            logger.error(f"从Milvus获取已处理记录失败: {e}")
            
        return processed_ids


class SmartProcessorV5_3:
    """智能处理器 V5.3 - Milvus查询优化版本"""
    ...
    def __init__(self):
        ...
        # 已处理记录管理器 - V5.3改进：传入Milvus连接器
        self.record_manager = ProcessedRecordManager(self.milvus)
    ...
    def get_processed_announcements(self, start_date: str = None, end_date: str = None, use_cache: bool = True) -> Set[str]:
        """获取已处理的公告ID - V5.3改进版，支持分批查询和日期过滤
        ...
        # 如果使用缓存，先尝试从本地记录管理器获取
        if use_cache:
            # 同步Milvus数据（如果需要）
            self.record_manager.sync_with_milvus(start_date, end_date)
            processed_ids = self.record_manager.get_all_processed_ids()
            self.console.print_success(f"已处理 {len(processed_ids)} 个公告（含缓存）")
            return processed_ids
        
        # 直接从Milvus查询（不使用缓存）
        try:
            print("\n直接从Milvus查询已处理记录（不使用缓存）...")
            processed_ids = self._get_processed_from_milvus_batch(start_date, end_date)
            self.console.print_success(f"已处理 {len(processed_ids)} 个公告（实时查询）")
            return processed_ids
            
        except Exception as e:
            self.console.print_error(f"获取已处理公告失败: {e}")
            logger.error(f"Milvus查询错误详情: {e}", exc_info=True)
            # 失败时尝试从本地缓存获取
            return self.record_manager.get_all_processed_ids()
    
    def _get_processed_from_milvus_batch(self, start_date: str = None, end_date: str = None) -> Set[str]:
        """从Milvus分批获取已处理的公告ID - V5.3新增方法
        ...
        """
        processed_ids = set()
        batch_size = 5000  # 每批查询5000条
        offset = 0
        total_batches = 0
        max_offset = 16384 - batch_size  # Milvus限制：offset+limit <= 16384
        ...
        try:
            while offset <= max_offset:
                ...
                # 显示进度
                print(f"\r检查已处理的公告... 第{total_batches}批，本批{len(results)}条，"
                      f"新增{batch_ids}个ID，总计{len(processed_ids)}个 "
                      f"(耗时{batch_time:.1f}秒)", end='', flush=True)
                ...
                # 如果已经达到Milvus的限制，警告并退出
                if offset >= max_offset and len(results) == current_limit:
                    print(f"\n⚠ 警告：达到Milvus查询限制(16384条)，可能有更多数据未查询")
                    logger.warning("达到Milvus offset限制，建议使用日期范围缩小查询范围")
                    break
                    
        except Exception as e:
            logger.error(f"Milvus批量查询失败: {e}")
            raise
    ...
    def process_unprocessed_announcements(self,
                                          ...
                                          task_type="未指定",
                                          use_cache=True):
        """处理未处理过的公告 - V5.3显示优化版本
        
        Args:
            start_date: 开始日期
            end_date: 结束日期
            use_content_filter: 是否使用内容过滤
            max_count: 最大处理数量
            task_type: 任务类型描述
            use_cache: 是否使用缓存（新增参数）
        """
        
        self.console.print_header(f"智能处理器 V5.3 - {task_type}")
        ...
        config_info = {
            ...
            "最大处理数": max_count,
            "查询模式": "缓存优先" if use_cache else "实时查询"
        }
        ...
        # 获取已处理的ID - V5.3改进：支持日期范围过滤
        processed_ids = self.get_processed_announcements(start_date, end_date, use_cache)
    ...
    def show_statistics(self):
        ...
        # 查询模式信息
        print(f"\n{Fore.BLUE}查询配置:{Style.RESET_ALL}")
        print(f"  Milvus批量大小: 5000条/批")
        print(f"  本地缓存同步: 每小时自动")
    ...
    def force_sync_with_milvus(self):
        """强制与Milvus同步 - V5.3新增功能"""
        self.console.print_header("强制同步Milvus数据")
        
        confirm = input(f"\n{Fore.YELLOW}这将从Milvus重新获取所有已处理记录，是否继续? (y/n):{Style.RESET_ALL} ")
        if confirm.lower() != 'y':
            self.console.print_warning("已取消")
            return
            
        start_time = time.time()
        self.record_manager.sync_with_milvus(force=True)
        
        sync_time = time.time() - start_time
        self.console.print_success(f"同步完成，耗时 {sync_time:.1f} 秒")
        self.console.print_info(f"当前已处理记录数: {len(self.record_manager.get_all_processed_ids())}")
...
def main():
    """主函数 - V5.3优化版"""
    processor = SmartProcessorV5_3()
    console = ConsoleHelper()
    
    while True:
        console.print_header("智能处理器 V5.3 - Milvus查询优化版")
        ...
        if choice == '1':
            ...
            use_cache = input(f"{Fore.YELLOW}使用缓存? (y/n, 默认y):{Style.RESET_ALL} ").lower() != 'n'
            ...
            print("\n查询最新的10条记录...")
            try:
                results = processor.milvus.collection.query(
                    expr="chunk_id == 0",
                    output_fields=["doc_id", "ann_date", "ts_code", "title"],
                    limit=10
                )
                
                print(f"\n找到 {len(results)} 条记录:")
                for i, r in enumerate(results):
                    print(f"\n{i+1}. {r.get('ts_code', 'N/A')} - {r.get('ann_date', 'N/A')}")
                    print(f"   文档ID: {r.get('doc_id', 'N/A')}")
                    print(f"   标题: {r.get('title', 'N/A')[:50]}...")
                    
                # 统计日期分布
                print("\n\n日期分布统计...")
                date_results = processor.milvus.collection.query(
                    expr="chunk_id == 0",
                ...
                for r in date_results:
                    date = r.get('ann_date', '')
                    if date:
                        year_month = date[:6]  # 取年月
                        date_counts[year_month] = date_counts.get(year_month, 0) + 1
                
                print("\n年月分布:")
                for ym, count in sorted(date_counts.items(), reverse=True)[:10]:
                    print(f"  {ym}: {count} 条")
                    
            except Exception as e:
                console.print_error(f"调试查询失败: {e}")
