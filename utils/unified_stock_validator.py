...
class UnifiedStockValidator:
    ...
    def _extract_stock_by_name_enhanced(self, question: str) -> Optional[Tuple[Optional[str], Optional[str]]]:
        ...
        try:
            ...
            name_patterns = [
                ...
                r'([一-龥]{2,6})-(?:UW|U|W)\b',
                # 带A/B后缀的股票（如万科A、南玻A、特力A、渝三峡A）- 增强边界
                r'([一-龥]{2,4}[AB])(?![A-Za-z])(?=的|主力|资金|行为|本|近|最近|散户|小单|中单|大单|超大单|$|\s)',
                ...
                # "XX的PE/财务/股价"等模式
                r'([一-龥]{2,6})(?:的)?(?:PE|PB|财务|股价|市盈率|市净率|分析|资金|年报|公告|涨幅|跌幅|排名|排行)',
                # "分析XX的"模式
                r'分析([一-龥]{2,6})的',
                # "解析XX的"模式
                r'解析([一-龥]{2,6})的',
                ...
                # "评估XX本周/本月"模式（优先级高）
                r'评估([一-龥]{2,6})本[周月年]',
                # "研究XX的"模式 - 增强版，支持A/B后缀
                r'研究([一-龥]{2,6}[A-Z]?)的',
                # "研究XX本月/本周"模式 - 支持时间段
                r'研究([一-龥]{2,6}[A-Z]?)本[月周年]',
                # "研究XX与/和"模式 - 用于多股票对比
                r'研究([一-龥]{2,6})[与和]',
                # "分析XX近N天"模式
                r'分析([一-龥]{2,6})近\d+天',
                ...
                # "XX主力"模式
                r'([一-龥]{2,6})主力',
                # "XX本周/本月"模式 - 加强版，支持万科A这类带字母后缀的股票
                r'([一-龥]{2,6}[A-Z]?)本[周月年]',
                # "XX近/最近N天"模式 - 加强版
                r'([一-龥]{2,6}[A-Z]?)(?:近|最近)\d+天',
                # 一般的2-6字中文（最后尝试）
                r'([一-龥]{2,6})(?=财务|分析|怎么样|如何|资金流向|最新股价|股价|年报|公告|涨幅|跌幅|排名|排行|前\d+天|最近)',
            ]
    ...
    def extract_multiple_stocks(self, question: str) -> List[str]:
        ...
        stock_patterns = [
            ...
            # 带A/B后缀的股票（增强版，支持更多边界）
            r'([一-龥]{2,4}[AB])(?![A-Za-z])(?=[和与及、，,]|$|\s|的|\d|vs|VS|从|到|至|本|近|最近|主力|资金|行为)',
            # ST和*ST股票
            ...
            # 带银行后缀的公司（特别处理）
            r'([一-龥]{2,6}银行)(?=[和与及、，,]|$|\s|的)',
            # 专门处理"XX和YY"模式中的第一个股票
            r'(?:评估|研究|分析|对比|比较)([一-龥]{2,6}(?:银行)?)[和与]',
            # 处理"与"作为连接词的情况
            r'([一-龥]{2,6})[与]([一-龥]{2,6})',
            # 普通中文股票名（重要！添加这个模式）
            # 不再排除动词前缀，因为会影响多股票提取
            r'([一-龥]{2,6})(?=[和与及、，,]|$|\s|的|\d|vs|VS|从|到|至|收入|利润|营收|成交|涨|跌|股价|市值|资金|排名|排行|前)',
        ]
        ...
        for pattern in stock_patterns:
            ...
            for match in matches:
                # 处理可能的元组结果（来自包含多个捕获组的正则表达式）
                if isinstance(match, tuple):
                    potential_stocks.extend([m for m in match if m])
                else:
                    potential_stocks.append(match)
